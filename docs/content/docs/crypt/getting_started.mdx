...
In your `start/env.ts` file, replace `Env.create` with `Crypt.create` and add your first secret.

```ts title="start/env.ts" twoslash
// @noErrors
import { Env } from '@adonisjs/core/env'
import { Crypt } from '@foadonis/crypt' // [!code highlight]

// [!code highlight]
export default await Crypt.create(new URL('../', import.meta.url), {
  ...
  DB_PASSWORD: Crypt.secret(), // [!code highlight]
})
```

## Generate your keypair

Crypt works with [assimetric cryptography](https://en.wikipedia.org/wiki/Public-key_cryptography). It will generate two keys, one for encrypting (public key) and one for decrypting (public key).
