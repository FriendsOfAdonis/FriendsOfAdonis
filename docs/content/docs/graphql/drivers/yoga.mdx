---
title: Yoga Driver
description: "Set up GraphQL Yoga through drivers.yoga(), enable file uploads with the AdonisJS bodyparser processManually list and a File scalar, and add Yoga plugins"
---
...
Yoga only answers requests whose URL matches its `graphqlEndpoint` option. The driver sets it to the `path` of your GraphQL configuration, so a custom path such as `/api/v1/gql` works without further setup. Pass `graphqlEndpoint` explicitly to `drivers.yoga()` to override it.
