---
title: Middlewares
description: "Intercept GraphQL operations in AdonisJS with middleware classes and guards, added with @UseMiddleware on methods, resolvers and fields or registered globally"
---
...
```ts title="app/graphql/middlewares/access_logger_middleware.ts" twoslash
// @noErrors
...
```ts title="app/graphql/middlewares/performance_logger_middleware.ts" twoslash
// @noErrors
import { inject } from '@adonisjs/core'
import { Logger } from '@adonisjs/core/logger'
import { NextFn, ResolverData } from '@foadonis/graphql'
...
```ts title="app/graphql/middlewares/competitor_middleware.ts" twoslash
// @noErrors
