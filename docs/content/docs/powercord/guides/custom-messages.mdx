...
Built-in messages can be overwritten allowing you to further extend Powercord by using `event.preventDefault()`

In this example the `log` message now uses [consola](https://github.com/unjs/consola) to log messages.

```js title="resources/js/app.js"
import { consola } from 'consola'
import Powercord from '@foadonis/powercord'

Powercord.on('dialog', (event) => {
  event.preventDefault()

  const { level, message } = event.details
