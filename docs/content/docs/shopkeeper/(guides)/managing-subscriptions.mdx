...
description: "Check subscribed() status, swap plans, adjust quantities, cancel and resume, and manage multi-price Stripe subscriptions with AdonisJS Shopkeeper."
icon: Repeat
---
...
<Callout type="warn">
  A subscription in `incomplete` state cannot be modified. Calling `swap` or `updateQuantity` will
  throw an exception until the payment is confirmed.
</Callout>
...
<Callout type="warn">
  When a subscription has multiple prices, the `stripePrice` and `quantity` attributes on the
  `Subscription` model are `null`. Use the `items` relationship to access individual price details.
</Callout>
