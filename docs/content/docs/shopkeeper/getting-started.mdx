...
description: "Take a first Stripe payment from AdonisJS: add @foadonis/shopkeeper, run its billing migrations, then compose the billable() mixin into your User model."
icon: Rocket
---
...
<Callout type="warn">
  The `STRIPE_WEBHOOK_SECRET` environment variable is mandatory in production. It secures the Stripe
...
<Callout>
  Stripe and Shopkeeper update subscription statuses via webhooks. There may be a short delay before
  the subscription is active in your database after the customer completes their payment.
</Callout>
