...
`match` runs the `active` callback when the feature resolves to a truthy value, and `inactive` otherwise. The full resolution API (`isActive`, `isInactive`, `value`, `values`, `match`, and the `allActive` / `someActive` batch helpers) is covered in [Resolving Features](/docs/flick/resolving-features).
