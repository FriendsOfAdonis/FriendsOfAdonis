...
<Callout>
  The `features` import comes from `#generated/features`, a barrel file produced at build time by the
  `indexFeatures()` hook. It lists every `*_feature.ts` file under `app/features/`.
</Callout>

<Callout type="warn">
  On a fresh install the barrel does not exist yet. Run `node ace build` (or start the dev server
  with `node ace serve`) once so the assembler hook generates `.adonisjs/server/features.ts` before
  TypeScript compiles `config/flick.ts`.
</Callout>
...
<Callout>
  The result is cached per scope identifier, so the same user keeps getting the same outcome until
  the cache entry is cleared. See [Drivers](/docs/flick/drivers) for cache configuration.
</Callout>
