...
export default class DeleteUserAction extends compose(
  BaseAction,
  AsController(),
  AsCommand()
) {
...
```ts title="❌ Bad — we lose traceability of the business logic" twoslash
// @noErrors
...
```ts title="✅ Good — we know that creating a user sends a welcome email" twoslash
// @noErrors
export default class CreateUserAction extends BaseAction {
  async handle(data: CreateUserData) {
    const user = await User.create(data)
