...
The result will be wrapped into the `rootLayout` configured in `config/osmos.ts`.
...
```tsx
import osmos from '@foadonis/osmos/services/main'
import MyCustomLayout from '#osmos/layouts/my_custom_layout'

const renderer = osmos.createRenderer()
...
Thanks to [Async Local Storage](https://docs.adonisjs.com/guides/concepts/async-local-storage#async-local-storage) the [HttpContext](https://docs.adonisjs.com/guides/concepts/http-context) is available inside your components, allowing you to access the request, response, session, etc.
...
Under the hood Osmos uses [Alpine.js](https://alpinejs.dev/) to provide client-side interactivity making it available for you as well.
...
[Alpine.js events feature](https://alpinejs.dev/directives/on) uses a naming that does not work with JSX. Luckily, Osmos brings a special syntax to handle this by allowing you to prefix event names with `$` instead of `x-on:`.
