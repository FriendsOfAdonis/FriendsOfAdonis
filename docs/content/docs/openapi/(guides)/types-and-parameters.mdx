...
description: 'Define OpenAPI schemas using TypeScript classes, primitives, arrays, enums, and raw schemas. Configure optional properties and handle circular dependencies.'
---
...
When using the `enum` option, the enum is attached directly to the schema.

By using `registerEnumType` method, the enum is instead added to the [components](https://swagger.io/docs/specification/v3_0/components/) as a named schema. This is useful when using code generation tools.

```ts
import { registerEnumType } from '@foadonis/openapi/decorators'
...
<Callout type="warning">
  When referencing an enum you **must use a thunk** `() => My Enum`
</Callout>
