---
title: 'Advanced'
description: "Customize how a Searchable model is indexed and serialized, then import, flush and query your search indexes with where clauses and pagination."
---
...
<Callout type="warn">
  Depending on your Search engine, you might as well have to change your index configuration in
  `config/magnify.ts` to match your new index name.
</Callout>
...
```ts
const posts = await Post.search('Magnify').whereIn('status', ['published', 'draft']).get()
```

<Callout type="warn">
  If your application is using Meilisearch, you must configure your application's filterable
  attributes before utilizing Magnify's "where" clauses.
</Callout>
...
```ts
const posts = await Post.search('Magnify').whereIn('status', ['published', 'draft']).paginate(15, 1)
```
