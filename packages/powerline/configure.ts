...
export async function configure(command: ConfigureCommand) {
  ...
  await codemods.updateRcFile((rcFile) => {
    rcFile.addProvider('@foadonis/powerline/powerline_provider')
  })

  await codemods.makeUsingStub(stubsRoot, 'config/powerline.stub', {})
...
function logSuccess(command: ConfigureCommand) {
  ...
  const name = c.yellow('@foadonis/powerline')
  command.logger.log('')
  command.logger.log(c.green('╭─────────────────────────────────────────╮'))
  command.logger.log(c.green(`│ ${foadonis} | ${name} │`))
  command.logger.log(c.green('╰─────────────────────────────────────────╯'))
  command.logger.log('╭')
  command.logger.log('│ Welcome to @foadonis/powerline!')
  command.logger.log('│ ')
  command.logger.log('│ Get started')
  command.logger.log('│ ↪  Docs: https://friendsofadonis.com/docs/powerline')
  command.logger.log('│ ')
  command.logger.log(
