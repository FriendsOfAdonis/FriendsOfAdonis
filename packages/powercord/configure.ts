...
export async function configure(command: ConfigureCommand) {
  ...
  await codemods.updateRcFile((rcFile) => {
    rcFile.addProvider('@foadonis/powercord/powercord_provider')
  })

  await codemods.makeUsingStub(stubsRoot, 'config/powercord.stub', {})
...
function logSuccess(command: ConfigureCommand) {
  ...
  command.logger.log(c.green('╰─────────────────────────────────────╯'))
  command.logger.log('╭')
  command.logger.log('│ Welcome to @foadonis/powercord!')
  command.logger.log('│ ')
  command.logger.log('│ Get started')
  command.logger.log('│ ↪  Docs: https://friendsofadonis.com/docs/powercord')
  command.logger.log('│ ')
  command.logger.log(
