...
export interface WithPasswordOptions extends GeneratePasswordResetTokenOptions {
  /**
   * The manager hashing the passwords and generating the reset
   * tokens, or a function returning it.
   *
   * Defaults to the password service of the application, read on
   * every call. Give one when you construct the manager yourself,
   * in tests for example.
   */
  manager?: ManagerReference<PasswordManager>
...
type WithPasswordRow = {
   ...
   */
  readonly $passwordManager: PasswordManager
}
...
> = Model & {
  ...
  /**
   * The manager hashing the passwords and generating the reset
   * tokens. It is resolved on every access, from the "manager"
   * option of the mixin or the password service of the application.
   * Override it on the model to give it a manager of its own.
   */
  get $passwordManager(): PasswordManager
 ...
 * - $passwordManager static getter to resolve the manager of the
 *   model
 *
  ...
  return function <Model extends NormalizeConstructor<typeof BaseModel>>(
    ...
    class WithPasswordImpl extends superclass implements WithPasswordRow {
      static get $passwordManager() {
        return resolveManager(manager, () => password, 'password')
      }

      get $passwordManager() {
        return (this.constructor as WithPasswordClass).$passwordManager
      }

      @beforeSave()
      static async hashPassword<T extends WithPasswordClass>(this: T, row: InstanceType<T>) {
        if (row.$dirty[passwordColumnName]) {
          ;(row as any)[passwordColumnName] = await this.$passwordManager.hashPassword(
      ...
      static async verifyCredentials<T extends WithPasswordClass>(
        ...
        if (!row) {
           ...
           */
          await this.$passwordManager.hashPassword(password)
          throw new E_INVALID_CREDENTIALS()
        }
         ...
         */
        if (rehashOnVerify && this.$passwordManager.needsRehash(current)) {
      ...
      static async verifyPasswordResetToken<T extends WithPasswordClass>(
        ...
        const token = await this.$passwordManager.verifyPasswordResetToken(value, { purpose })
      ...
      static async resetPassword<T extends WithPasswordClass>(
        ...
        await this.$passwordManager.invalidateAllPasswordResetTokens(
      ...
      verifyPassword(plainPassword: string) {
        ...
        return this.$passwordManager.verifyPassword(password, plainPassword)
      }
      ...
      async generatePasswordResetToken(options: GeneratePasswordResetTokenOptions = {}) {
        return this.$passwordManager.generatePasswordResetToken(
      ...
      async invalidatePasswordResetTokens(options: InvalidatePasswordResetTokensOptions = {}) {
         ...
         */
        const purpose = 'purpose' in options ? options.purpose : defaults.purpose
        return this.$passwordManager.invalidatePasswordResetTokens(
          primaryKeyOf(this, 'invalidate the password reset tokens of'),
          { purpose }
        )
      }
