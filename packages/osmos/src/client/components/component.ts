...
export class HTMLComponentElement extends HTMLElement {
  ...
  get componentId() {
    const id = this.getAttribute('osmos:id')
    if (!id) throw new Error('The fragment does not have osmos:id')
    return id
  }

  get component() {
    const component = this.getAttribute('osmos:component')
    if (!component) throw new Error('The fragment does not have osmos:component')
    return component
  }
   ...
   */
  #refreshData() {
    const data = this.getAttribute('osmos:data')
    if (!data) return {}
